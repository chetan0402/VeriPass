...
func (ac *AdminCreate) SetHostel(s string) *AdminCreate {
	ac.mutation.SetHostel(s)
	return ac
}

// SetCanAddPass sets the "can_add_pass" field.
func (ac *AdminCreate) SetCanAddPass(b bool) *AdminCreate {
	ac.mutation.SetCanAddPass(b)
	return ac
}
...
func (ac *AdminCreate) check() error {
	...
	if _, ok := ac.mutation.Hostel(); !ok {
		return &ValidationError{Name: "hostel", err: errors.New(`ent: missing required field "Admin.hostel"`)}
	}
...
func (ac *AdminCreate) createSpec() (*Admin, *sqlgraph.CreateSpec) {
	...
	if value, ok := ac.mutation.Hostel(); ok {
		_spec.SetField(admin.FieldHostel, field.TypeString, value)
		_node.Hostel = value
	}
	if value, ok := ac.mutation.CanAddPass(); ok {
